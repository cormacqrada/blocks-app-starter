...
export class BlocksRendererElement extends HTMLElement {
    ...
    render() {
        ...
        const doc = deserializeFromMarkdown(id, markdown);
        const baseTree = doc.tree.blocks.length || doc.tree.collections.length
            ? doc.tree
...
function findApplyTokenCollectionBlock(tree) {
    return tree.blocks.find((b) => b.properties?.element === "applyTokenCollection") ?? null;
}
function applyThemeFromTree(tree, shadow) {
    const applyBlock = findApplyTokenCollectionBlock(tree);
    if (!applyBlock)
        return;
    const props = applyBlock.properties;
    const collectionName = props.collectionName;
    const outputType = props.outputType;
    if (!collectionName || outputType !== "cssVariables")
        return;
    const collection = tree.collections.find((c) => c.name === collectionName);
    if (!collection)
        return;
...
export function renderVisualBlock(block) {
    ...
    switch (kind) {
        ...
        default: {
            ...
            p.innerHTML = marked.parseInline(text);
            el.appendChild(p);
            return el;
        }
    }
...
function ensureStyles(shadow) {
    if (shadow.querySelector("style[data-blocks-style]"))
        return;
...
export function registerBlocksRenderer(tagName = "blocks-renderer") {

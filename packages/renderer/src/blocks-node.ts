...
export class BlocksNodeElement extends HTMLElement {
  private node: NodeBlock | null = null;
  ...
  private render(): void {
    ...
    title.textContent = String((this.node?.properties as any)?.title ?? this.node?.id ?? "Node");
    ...
    const ports = (this.node?.properties as any) ?? {};
    const inputPorts: NodePortDefinition[] = (ports.inputs as NodePortDefinition[]) ?? [];
    const outputPorts: NodePortDefinition[] = (ports.outputs as NodePortDefinition[]) ?? [];
  ...
  private attachDragBehavior(handle: HTMLElement): void {
    let dragging = false;
    let startX = 0;
    let startY = 0;
    let startNodeX = 0;
    ...
    const onPointerUp = (ev: PointerEvent) => {
      ...
      this.dispatchEvent(
        new CustomEvent<NodeMoveDetail>("node:moveRequested", {
          detail,
          bubbles: true,
          composed: true
        })
...
export function registerBlocksNode(tagName = "blocks-node"): void {
  if (!customElements.get(tagName)) {
    customElements.define(tagName, BlocksNodeElement);
  }
}

function ensureNodeStyles(shadow: ShadowRoot): void {
  if (shadow.querySelector("style[data-blocks-node-style]")) return;

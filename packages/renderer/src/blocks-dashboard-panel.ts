...
export class BlocksDashboardPanelElement extends HTMLElement {
  private panel: DashboardPanelBlock | null = null;
  private columns = 12;
  private rowHeightPx = 80;
  ...
  setConfig(panel: DashboardPanelBlock, columns: number, rowHeightPx: number): void {
    ...
    const props = panel.properties as any;
    this.x = props.x ?? 0;
    this.y = props.y ?? 0;
    this.w = props.w ?? 3;
    this.h = props.h ?? 2;
  ...
  private render(): void {
    ...
    const title: string = (this.panel?.properties as any)?.title ?? this.panel?.id ?? "Panel";
  ...
  private attachDragBehavior(handleEl: HTMLElement): void {
    let dragging = false;
    let startX = 0;
    let startY = 0;
    let startCol = 0;
    ...
    const onPointerMove = (ev: PointerEvent) => {
      if (!dragging) return;
      const grid = this.parentElement as HTMLElement | null;
      if (!grid) return;
    ...
    const onPointerUp = (ev: PointerEvent) => {
      ...
      const detail: DashboardPanelMoveDetail = {
        panelId,
        x: newX,
        y: newY,
        w: this.w,
      ...
      this.dispatchEvent(
        new CustomEvent<DashboardPanelMoveDetail>("panel:moveRequested", {
          detail,
          bubbles: true,
          composed: true
        })
  ...
  private attachResizeBehavior(resizeEl: HTMLElement): void {
    let resizing = false;
    let startX = 0;
    let startY = 0;
    let startW = 0;
    ...
    const onPointerUp = (ev: PointerEvent) => {
      ...
      const detail: DashboardPanelResizeDetail = {
        panelId,
        x: this.x,
        y: this.y,
        w: newW,
      ...
      this.dispatchEvent(
        new CustomEvent<DashboardPanelResizeDetail>("panel:resizeRequested", {
          detail,
          bubbles: true,
          composed: true
        })
...
export function registerBlocksDashboardPanel(tagName = "blocks-dashboard-panel"): void {
  if (!customElements.get(tagName)) {
    customElements.define(tagName, BlocksDashboardPanelElement);
  }
}

function ensurePanelStyles(shadow: ShadowRoot): void {
  if (shadow.querySelector("style[data-blocks-dashboard-panel-style]")) return;
